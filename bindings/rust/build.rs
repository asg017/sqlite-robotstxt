...
fn download_static_for_platform(version: String, output_directory: &PathBuf) {
    ...
    let base = "https://github.com/asg017/sqlite-robotstxt/releases/download";
    let url = match platform {
        Platform::MacosX86_64 => {
            format!("{base}/{version}/sqlite-robotstxt-{version}-static-macos-x86_64.tar.gz")
        }

        Platform::MacosAarch64 => {
            format!("{base}/{version}/sqlite-robotstxt-{version}-static-macos-aarch64.tar.gz")
        }
        Platform::LinuxX86_64 => {
            format!("{base}/{version}/sqlite-robotstxt-{version}-static-linux-x86_64.tar.gz")
        }
        Platform::WindowsX86_64 => {
            format!("{base}/{version}/sqlite-robotstxt-{version}-static-windows-x86_64.zip")
        }
    };
...
fn main() {
    ...
    let output_directory = if cfg!(feature = "download-libs") {
        let output_directory = std::path::Path::new(std::env::var("OUT_DIR").unwrap().as_str())
            .join(format!("sqlite-robotstxt-v{version}"));
        ...
        std::env::var("LIB_SQLITE_robotstxt").expect("The LIB_SQLITE_robotstxt environment variable needs to be defined if the download-libs feature is not enabled").into()
    };
    ...
    if cfg!(feature = "robotstxt") {
        println!("cargo:rustc-link-lib=static=sqlite_robotstxt0");
    }
    if cfg!(feature = "hola") {
